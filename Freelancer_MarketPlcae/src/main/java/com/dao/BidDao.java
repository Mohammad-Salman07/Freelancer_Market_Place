...
public class BidDao {
	...
	public List<Bid> getBidsByProjectId(int projectId) {
		...
		String sql = "SELECT b.*, f.fullname AS freelancer_name, f.email AS freelancer_email, p.client_id "
				+ "FROM bids b " + "JOIN freelancers f ON b.freelancer_id = f.id "
				+ "JOIN projects p ON b.project_id = p.id " + "WHERE b.project_id = ?";
	...
	public List<Bid> getActiveBidsByFreelancerId(int freelancerId) {
		List<Bid> list = new ArrayList<>();
		String sql = "SELECT b.*, p.title AS project_title, p.client_id AS client_id " + "FROM bids b "
				+ "JOIN projects p ON b.project_id = p.id " + "WHERE b.freelancer_id = ? AND p.status != 'completed'";
		try {
			PreparedStatement ps = conn.prepareStatement(sql);
			ps.setInt(1, freelancerId);
			ResultSet rs = ps.executeQuery();
